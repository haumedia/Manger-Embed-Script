...
// Subtitle
	$sub = $conn->prepare("SELECT * FROM `subtitle` WHERE `tmdb`='$id'");
		$sub->execute();
        $tracks = '';		
		$index = 0; 
		while($row = $sub->fetch()){
...
if(isset($url) && strpos($url,"dailymotion.com") !== false){
$curl = new cURL();
...
<div class="media-title"><div class="show-name"><?php echo $moviestitle;?></div></div>
<?php if($adblock == 1){ ?>
